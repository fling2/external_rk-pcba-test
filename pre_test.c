...
int start_test_pthread(struct testcase_info *tc_info)
{
	...
	else if(!strcmp(tc_info->base_info->name, "SPDIF"))
	{
		spdif_err = pthread_create(&spdif_tid, NULL, spdif_test, tc_info); //
		if(spdif_err != 0)
		{  
		   printf("create lan test thread error: %s/n",strerror(spdif_err));
		   return -1;
